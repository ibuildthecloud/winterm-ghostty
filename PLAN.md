...
| 8 — Accessibility & packaging | packaging half done; accessibility not started | [0009](docs/sessions/0009-phase-8.md): **entered early at the gate's direction, with Phase 7's presentation half still open.** Signed sideloadable x64 MSIX published as [v0.1.0](https://github.com/ibuildthecloud/winterm-ghostty/releases/tag/v0.1.0); `scripts/package.ps1` builds/signs/stages it. Fork ships under its own identity (`WintermGhostty`, `CN=Darren Shepherd`, alias `wtg.exe`) — upstream's `CN=Microsoft Corporation` is unsignable by us and its `Name` would have replaced a real WT Dev install. Signing/identity open question resolved: **self-signed test cert**. **Unverified: nobody has installed the artifact** — the session had no elevation, so the trust + `Add-AppxPackage` path is untested. ARM64 still blocked by the Zig 0.16.0 `aarch64-windows-msvc` bug. UIA/Narrator untouched. |

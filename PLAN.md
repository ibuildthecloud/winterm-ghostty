...
| 7 — Presentation & performance | in progress — performance done, presentation untouched | [0008](docs/sessions/0008-phase-7.md), 2026-08-06/07. **The pane, not the harness.** Phase 3's "2.9x slower than WT" measured `hwnd-host`; a pane measured **4.2 MB/s against cascadia's 38.1**. The whole gap was `_connectionOutputHandler` calling `ghostty_surface_render_now` **per chunk of pty output** — the Phase 5 repaint-freeze workaround, which forces hundreds of synchronous renders to show one final frame. Throttled (`til::throttled_func`, 8 ms, trailing-only): **28.0 MB/s median**. The remaining ~25% needs libghostty to re-arm its wakeup so the forced render can go entirely — upstream, not attempted. Three measurements were wrong first, each convincingly: other GPU load moved the same binary 15.2 → 12.3 MB/s; `profiles.defaults.engine` was already `ghostty`, so a "cascadia vs ghostty" run **measured ghostty twice** (the user caught it, nothing here did); and "everything was Debug on both sides" was asserted and false — Phase 3's 14.9 was already ReleaseFast. **`?` could not be typed into a ghostty pane** — it arrived as `/`, and at speed the damage spread: `echo A?B/C` typed quickly produced `????ECH?O A??B?C`, with Shift smeared onto neighbouring keys, while a **cascadia pane in the same window given the identical keystrokes was perfect**. Cause: `TrySendKeyEvent` translated the key under `GetKeyboardState()`, which reports the state as of the last message the *calling thread* retrieved, and XAML raises `KeyDown` some way behind its message pump. Cascadia is immune structurally, not by luck — it never calls `ToUnicode`, its text comes from `CharacterReceived`, which Windows generated with the right state. Now translated under the modifiers that came **with the event**; moved into `GhosttyKeyTranslator` as a pure function so a test can reach it. **libghostty was cleared before WT was blamed**, by a new `GHOSTTY_HARNESS_KEY` probe that synthesizes the exact event a keystroke produces — `?` encodes to `0x3f`. `consumed_mods = NONE` had been the leading suspect, reasoned from `key.zig`; it is not the cause, and the probe settled in one run what source-reading had got wrong. **Typing speed is a variable, not a nuisance**: at 200 ms/character the pane was already correct *before* the fix, so a slow test would have reported it fixed while it was not. The new unit test **fails against the old code for the reason the bug existed** — no key is held during a unit test, so a `GetKeyboardState` implementation sees Shift up and answers `/`. Suites: `unitControl` 62 → **63/63**, smoke 9 → **10 checks**, 39 → **42** terminal patches. Open: the Settings UI dropdown is still unseen by a human. **Repaint re-checked 2026-08-07 and passes** — `dir /s C:\Windows\System32` + a marker echo, photographed mid-scroll and at completion with no scroll/resize/keystroke between; the last batch reaching the screen is the whole assertion, since the freeze's signature is that it does not. Focused pane only; the **unfocused** half, which is where the original bug was found, still wants a human. **Readiness done** — [phase-7-readiness](docs/phase-7-readiness.md), all three open questions answered against source. **The waitable-swapchain question had a wrong premise**: it offered "xev async bridge" as an option, and libxev *cannot wait on a Win32 event handle at all* — `associate_fd` is `CreateIoCompletionPort`, which does not accept an auto-reset event, and there is no `RegisterWaitForSingleObject` anywhere in the pinned dep. Cascadia answers the same question in this tree without an event loop: waitable flag, `SetMaximumFrameLatency(1)`, and a plain blocking `WaitForSingleObjectEx` on the render thread guarded against the auto-reset double-wait. **Latency has never been measured on either engine**, and the recommended instrument is a keystroke-to-pixel probe built on `wgc-shot`'s frame pool using WGC `SystemRelativeTime` — same instrument both sides, so the DWM bias cancels. **DECISION-NEEDED: exit criterion 1 is unmeetable as written** — "0 presents/sec with cursor blinking" is a contradiction, since changed pixels require a present; proposed restatement is "presents nothing when nothing changed, and a blink presents only the cursor cell's dirty rect", which is what cascadia does (its blink timer deliberately does *not* `TriggerRedrawAll`). Ours currently costs a **full rebuild and full-surface present every 600 ms on a completely idle pane**, because `rebuildCells` sets `cells_rebuilt = true` unconditionally and that alone drives `needs_redraw` — the row-level dirty state that already exists gates rebuild work and never presentation. Making that flag honest is the cheapest item in the phase. Also found: **there is no periodic draw at all** in our configuration (the 8 ms timer arms only for custom shaders), and `ghostty_surface_render_now` runs the whole rebuild-and-present **on the caller's thread**, so WT's throttle is doing that on a timer thread concurrently with the renderer. **CORRECTION 2026-08-07: the wakeup-coalescing explanation is wrong.** The readiness doc claimed a notify arriving after the render callback samples state is dropped. It is not: the loop swaps the flag false before the callback, but a notify during the callback re-sets it *and* posts a packet, `.rearm` pushes the completion back **without clearing the flag** (`iocp.zig:740-743`), and `wakeupCallback` always re-arms — so the next drain runs it again. Phase 5's "80 notifies -> <20 wakes" is coalescing working as designed, not a bug; the symptom that mattered was rendering stopping *entirely*, which coalescing does not explain. **The Phase 5 freeze therefore has no established cause**, and `render_now`, the throttle and the 26% gap are a workaround for an undiagnosed fault. The fork comment at `Surface.zig:916-937` carries the same wrong claim. **Third sitting: the freeze had the wrong cause for three phases.** On Windows, pty output had **never woken the renderer** - `Termio`/`Options`/`StreamHandler` held `renderer_wakeup: xev.Async` *by value*, and libxev's IOCP async keeps its state in the struct, so a copy has `waiter == null` and `notify()` sets a bool nobody reads **and returns success**. Silent, Windows-only, invisible to every test. Found by measuring (patch 33's stage counters, reported from the *cursor blink timer* so the report survives the very stall it is watching): `wakeup` +2/sec before the fix, **+321/sec** after. Patch 34 holds it by pointer. This explains why the freeze needed an *unfocused* pane (blink timer cancelled on focus loss -> zero wakeups), why scrolling cured it and resizing did not, and why no native apprt sees it. **The forced render and its throttle are deleted** (terminal patch 44). **RETRACTED: "forced render removed = 37.4 MB/s = parity" measured a terminal that was not drawing.** Corrected, same session: ghostty **30.9, 30.9, 30.9**; cascadia 30.3, 37.7, 39.0, 32.3 - a ~30% spread, so *cascadia* is now the shakier number. Gap ~10-15%, not zero and not 25%. Two instruments proved untrustworthy: the engine-identity check (ghostty-internal.dll loads even for a cascadia pane when defaults are ghostty; use the search box's greyed toggles) and the diagnostic itself (20.2 MB/s with `GHOSTTY_RENDER_DIAG=1` vs 30.9 without - never measure through it). **Latency baseline exists, and the criterion is not met.** `harness/keylatency` measures keystroke-to-pixel with the same instrument on both engines (QPC before `SendInput` vs the `SystemRelativeTime` of the first captured frame that changed), so the DWM bias it includes is common to both. 20 rounds each, same session: **ghostty median 51.0 ms** (min 39.5, max 68.1) against **cascadia 41.4 ms** (min 23.9, max 55.0) - about 10 ms and ~24% worse. Candidate, not yet a finding: 10 ms is half a frame at 60 Hz, and the one latency thing cascadia does that we do not is the waitable swap chain with `SetMaximumFrameLatency(1)`; we present `SyncInterval = 1` with two buffers and no frame-latency object. That makes the waitable swap chain the next item and `docs/perf/latency.md` the way to tell whether it helped. **Criterion 3 (usable over RDP/WARP, no fallback dialogs) MET**, in a pane, over a live RDP session - Phase 3 had verified forced WARP in the *harness* only, which is a different presentation path (hwnd vs composition). A GPU is exposed to the remote session here, so a pane picks `driver=hardware feature_level=11_1` over RDP; forced WARP gives `driver=warp feature_level=11_1`, no dialog either way, and the two renders were **compared rather than eyeballed**: 344,199 sampled pixels, 0 differing by more than 8, max channel delta 1. **And WARP was finally characterised**: throughput 26.8 vs hardware 28.6 (6% slower) but latency **93.5 ms vs 45.9** - more than double. A CPU rasterizer keeps up with a flood and is poor at answering one keystroke. That is the shape that makes cascadia's `BackendD2D` ("low latency remote desktop and older/no GPUs") a *latency* argument rather than a throughput one - worth knowing before deciding we never need a second backend. Untested: a genuinely GPU-less machine, where WARP is fallen back to rather than forced. **Two gaps found on the idle criterion, both recorded and neither fixed (KD-04, KD-05).** Asked whether an unfocused pane presents 0/sec: **it does not** - measured, the counters climb by 2/sec unbroken through 12 seconds of the pane being in the background. ghostty cancels the blink timer on focus loss correctly and our `LostFocus` forwards it correctly; **XAML never raises `LostFocus` when another *window* takes the foreground**, so the signal never arrives. Phase 6's unfocused-cursor verification exercised pane-to-pane focus inside one window - window deactivation was never tried. Separately, **the system's cursor-blink settings are ignored entirely**: cascadia reads `SM_CARETBLINKINGENABLED` and `GetCaretBlinkTime()`; our translator has no mention of blink, so disabling cursor blinking in Windows - an accessibility setting - has no effect on a ghostty pane. **Damage tracking is *not* the lever here and would not reduce the present count at all** - a visible blink must present; damage only shrinks each one. And `cells_rebuilt` honesty would not either: the `!needs_redraw` path still calls `presentLastTarget()`. **Tooling gap: `bench-throughput.ps1` drives `hwnd-host` only** — every pane number this phase is judged on was hand-taken outside it. **CORRECTION 2026-08-11, KD-04 fixed:** the sentence above — "XAML never raises `LostFocus` when another *window* takes the foreground" — is **wrong**, and this row's own numbers said so (`notify` frozen at 5 while `wakeup` climbed: a focus change queues a render, so the surface was never told anything in *either* direction). Traced: `LostFocus` does arrive on window deactivation and the blink does stop. The real fault is at the other end — XAML's routed `GotFocus` fires for a control in a window that was **never activated**, since logical focus lives inside the app's tree, and nothing can contradict it afterwards. `WM_ACTIVATE` cannot be the gate either: it reports the active window of the thread's input queue, and was measured saying `1` while the foreground was another application's window. `TermControl::_pushCoreFocus` now gates on `GetForegroundWindow() == OwningHwnd()`, with activation and routed focus as triggers. Measured after: 1.7 presents/sec in front, ~0 in the background, and a never-in-front window never blinks at all. `scripts\probe-idle-focus.ps1` is the instrument. |
| 8 — Accessibility & packaging | packaging done (portable, in CI); accessibility not started | [0009](docs/sessions/0009-phase-8.md): fork ships under its own identity on two channels with **eight fresh CLSIDs**; `.github/workflows/release.yml` reconstructs both forks from the pins plus patches and publishes. **[v0.2.2](https://github.com/ibuildthecloud/winterm-ghostty/releases/tag/v0.2.2) was current at the time of writing** (v0.2.3 followed on 2026-08-10, v0.2.4 on 2026-08-12); v0.2.0 was withdrawn for KD-07. Three defects found by *using* the artifact, all fixed: KD-07 (1 MB stack vs Zig's 16 MiB assumption - the fourth instance of the 2026-08-04 note, now fixed in the exe rather than by another opt-in wrapper), KD-08 (`consumed_mods` unset, so `?` encoded as `/` under the kitty protocol), KD-06 (cursor movement dirties no row, so a ligature kept stale shaping). `scripts/smoke-release.ps1` is the release gate and **asserts the engine from the process under test** - three of its early passes were cascadia panes. Signing still unconfigured, so releases are portable-only and the MSIX path stays unverified. ARM64 still blocked by the Zig `aarch64-windows-msvc` bug. UIA/Narrator untouched. |
| 9 — Upstreaming loop | ongoing/background | |
| — | — | **User-reported fix, 2026-08-17 (KD-11, terminal patch 0054):** **alt+drag did not block-select** on a ghostty pane. `GhosttyControlCore::LeftClickOnTerminal` took `altEnabled` and named it `/*altEnabled*/`, and every mouse event the selection path sent carried `GHOSTTY_MODS_NONE`. ghostty decides rectangle-vs-linear **per mouse event**, from `SurfaceMouse.isRectangleSelectState`, which off macOS reads `ctrlOrSuper() and alt` — so forwarding WT's alt faithfully would not have fixed it either. The alt state is now latched at the click (cascadia's semantics) and sent as ctrl+alt on every mouse **position**; the presses stay bare, because a press reads the same bits as `if (mods.ctrlOrSuper()) .output else .line` and would have turned alt+triple-click into "select the command's output". Measured on the engine, not reasoned — new `GHOSTTY_HARNESS_SELECT_BLOCK`: `"CDE<LF>MNO<LF>WXY"` with the modifiers against `"CDEFGHIJ<LF>KLMNOPQRST<LF>UVWXY"` without. unitControl 63 → **65/65**, smoke 10 → **11 checks**. **Confirmed in a pane by the user**, ghostty and cascadia side by side in one window: alt+drag block-selects on both. It had to be a human check — the synthetic-mouse driver written for it produces no selection on a **cascadia** pane either, so it measures the driver, not the engine. |
| — | — | **User-reported fix, 2026-08-18 (KD-22, ghostty patch 0044 + terminal patch 0061):** **a ghostty pane never learned the shell's working directory**, so a duplicated tab or split opened at the profile's `startingDirectory` rather than where the shell was. Not a missing sequence: libghostty's OSC parser already routes all three spellings — OSC 7's `file:` URI, ConEmu's `OSC 9;9` and iTerm2's `OSC 1337;CurrentDir` — to one `report_pwd` command, and `GhosttyControlCore` has handled `GHOSTTY_ACTION_PWD` since Phase 5. `termio/stream_handler.zig`'s `reportPwd` returned early on Windows with *"unimplemented"*, before `setPwd` and before the surface message, so the action was never dispatched. The Windows path now takes a URI **or** a bare native path and always stores a native one, validating the URI's host as local (RFC 8089 makes an empty host this machine, which is the `file:///C:/src` form shell integration emits) and refusing anything not drive-absolute or UNC. **Measured twice**: through the DLL with a new `GHOSTTY_HARNESS_FEED` inventory run of all 23 OSCs WT dispatches, and through a real pane with no keyboard — persisted-layout `state.json` reports `…\pwd-conemu` and `…\pwd-osc7` where the profile said something else. The same run measured the rest of WT's custom set: `OSC 9;4` taskbar progress already arrives; `OSC 133` marks, `OSC 633` completions and WT's proprietary `OSC 9001;CmdNotFound` do not, and the last two need new libghostty API — escalated, not decided in passing. **Desktop notifications wired up the same day at the user's direction** (terminal patch 0062, GD-19): `ShowNotification` is raised for `OSC 777;notify`, gated on the profile's `compatibility.allowOSC777` — off by default — translated to ghostty's `desktop-notifications`, whose default is the opposite, so the inverted defaults are pinned by a unit test. Everything downstream (toast, rate limit, focus rule, click-to-summon) is `TerminalPage`'s and unchanged. New `scripts/probe-notify.ps1` measures it both ways round off the debugger channel, because a suppressed notification and one never raised look identical from outside. |
| — | — | **User-reported fix, 2026-08-18 (KD-23, terminal patch 0063):** **`compatibility.allowOSC52` was not honoured on a ghostty pane** — the profile switch that decides whether a program may put text on the clipboard was never translated to ghostty's `clipboard-write`. Invisible for eight phases because both sides default to allowing it: it bit only the user who turned OSC 52 off on purpose. The risk was that `clipboard-write` also gated ghostty's *own* copy, which would have cost that user their ctrl+c — measured through the DLL rather than argued: with `deny`, an `OSC 52` write is refused while the `copy_to_clipboard` binding action `GhosttyControlCore` sends still delivers. `ask` is deliberately never sent (`GhosttyEngine`'s write callback implements no prompt, so it would read as "confirm" and behave as "write"), and the read half needs nothing on either engine. The same question asked of the whole `compatibility.*` family: `allowDECRQCRA` is moot, and **`allowDECNKM` and `kittyKeyboardMode` are open** — both need a new libghostty config, escalated rather than decided. `allowDECNKM` defaults *off*, so that one differs for everyone. |
| — | — | **User-reported fix, 2026-08-21 (KD-24, terminal patch 0064):** **after the child exits, ctrl+d and enter did nothing on a ghostty pane** - the pane printed the connection's `[process exited with code 56] You can now close this terminal with Ctrl+D, or press Enter to restart` and then ignored both keys, leaving the mouse as the only way out. Both are the *host's* keys: cascadia answers them in `ControlCore::SendCharEvent`, which a ghostty pane never reaches - `Terminal::SendKeyEvent` returns no output for a key the layout maps to a character, so cascadia gets a `CharacterReceived` event, while `ghostty_surface_key` encodes Enter and ctrl+d itself and reports them handled. Adding the branch to `GhosttyControlCore::SendCharEvent` alone would have looked right and changed nothing. Answered on the key path instead, ahead of the surface and ahead of the read-only guard (a read-only pane whose process is gone still has to close), with the release swallowed alongside the press; `GhosttyExitKeyFor` is a pure function beside the other two key rules, unit tested with no surface. **Measured in the shipped product** by posting `WM_KEYDOWN` to the XAML input site - no focus steal, which mattered because the user was at the keyboard: Enter restarts in place (a second exit message, scrollback intact) and ctrl+d closes the pane, while a *live* pane still sends both to the shell. **Open, and escalated:** `GhosttyControlCore::HardResetWithoutErase` is a no-op, so a restart inherits any modes the dead client left on - libghostty's only reset erases, and a new C entry point is a public API shape. unitControl 70 -> 71. |
| — | — | **User-reported fix, 2026-08-21 (KD-25, terminal patches 0065 + 0066):** **no shifted character could be typed into a ghostty pane from the Windows App on Android** - `?` arrived as `/`, `A` as `a`, `:` not at all. Not a regression (0.2.2 the same, checked by the reporter) and invisible at the machine's own keyboard, which is why eight phases missed it. **Two independent defects, and the first explanation was wrong about the transport while accounting for the symptom perfectly.** (1) `TermControl` samples the modifier keys with `CoreWindow::GetKeyState` when XAML raises `KeyDown`, which answers *now* rather than *when this key was pressed*; a client that presses and releases shift around a key in one burst is asked after the release, so `ToUnicode` is given no shift. Fixed by tracking the modifier keys from their own key events - ordered, so they cannot drift - and treating the snapshot as a floor (`GhosttyModifiersFor`), cleared on both `GotFocus` and `LostFocus` so a release delivered to another window cannot leave shift stuck down. (2) **The reporter's client does not send shift at all.** `harness/keylog`, a new low-level hook, measured what actually arrives: the soft shift key is a press of its own that ends 388 ms *before* the character, and the character is injected as `VK_PACKET` with the UTF-16 unit in the scan code. Read as a scan code, `0x41` is F7, `0x3F` is F5, `0x3A` is Caps Lock - so `A` sent F7's escape sequence and `:` sent nothing, with the character event suppressed because the key was reported handled. Fixed by `GhosttyPacketText`: the character is the event, keycode 0, surrogate pairs held across the two packets. **Two rounds of inference from source produced a confident wrong cause before the pane was instrumented** - the fix that ships with it, `GHOSTTY_TRACE_KEYS` (read by `scripts/probe-key-trace.ps1`), is what settled it, since the pane's behaviour is identical under several different events and only the pane can say which arrived. Measured with `harness/rawin` and `scripts/probe-shift-posted.ps1`, which posts key messages to the XAML input site and so never fights a user at the keyboard: posted packets for `; : ? A a` gave `;`, nothing, F1, F5 and F7 before, and `; : ? A a` exactly after. unitControl 71 -> **74/74**. **The report is NOT resolved, and the control run that would have said so was never taken until the end.** After both fixes shipped, `A?:` typed from Android into a patched pane arrived as `a`, `/`, `;` - the clean unshifted characters, which neither defect above can produce - and the reporter sees the same in a **cascadia** pane in the same window, which is stock WT code and therefore clears both engines. The leading hypothesis is now the client: `keylog` shows its on-screen shift as a *latch* released 388 ms before the character, and a latch nobody applies is unrecoverable at this end. Outstanding measurement: `keylog` while a single `A` is typed from the phone. Workaround to try first: Windows StickyKeys, which is built for exactly that sequence. **Two fixes were built, measured, documented and committed before one cascadia pane on the reporter's own transport was ever looked at.**

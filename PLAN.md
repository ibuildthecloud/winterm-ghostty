...
| — | — | **User-reported fix, 2026-08-21 (KD-25, terminal patches 0065-0067):** **no shifted character could be typed into a ghostty pane from the Windows App on Android** - `?` arrived as `/`, `A` as `a`, `:` not at all. Not a regression (0.2.2 the same) and invisible at the machine's own keyboard. **Three defects, and only the third was the report; the first two were built, measured, documented and committed while the symptom carried on unchanged.** (1) The modifier snapshot `TermControl` takes with `CoreWindow::GetKeyState` is later than the key it comes with, so a client that presses and releases shift in one burst loses it - fixed by tracking the modifier keys from their own ordered key events and treating the snapshot as a floor (`GhosttyModifiersFor`), cleared on both focus transitions. (2) A `VK_PACKET` carries a character with the UTF-16 unit in the *scan code*; read as a scan code, `0x41` is F7 and `0x3A` is Caps Lock, so `A` sent an escape sequence and `:` sent nothing - fixed by `GhosttyPacketText`, surrogate pairs included. (3) **The actual cause:** the pane never sees the packet. The character is producible on the layout, so Windows resolves it to a bare virtual key with **scan code 0** and no modifier - the on-screen shift having been released 388 ms earlier as a press of its own - and puts the real character in the message that follows. Asking the layout what key `0x41` produces answers `a` whatever was meant. `GhosttyKeyIsInjectedCharacter` reports such a key unhandled so the character event delivers it, exactly as cascadia does; inert for physical keyboards, which always carry a scan code, and narrowed to keys that produce text so injected arrows and function keys keep the engine's encoding. **Confirmed by the reporter from the phone**: `A?:` in, `A?:` at the child. **The diagnosis was one comparison, available from the first minute and not taken until the reporter supplied it himself** - a cascadia pane and a ghostty pane in one window, same keystrokes, same transport. Its absence produced two wrong-but-plausible causes and, briefly, a documented conclusion that the client was at fault. What no outside instrument could have shown: `keylog` sees a `VK_PACKET` on the wire and the pane sees virtual key `0x41` - **the OS rewrites the event in between**, so only the pane's own trace (`GHOSTTY_TRACE_KEYS`, new and shipped) could name it. unitControl 71 -> **75/75**.

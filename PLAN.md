...
| 3 — Real terminal rendering | in progress — 0/3 criteria | [0004](docs/sessions/0004-phase-3.md): **the harness is an interactive terminal.** `cmd.exe` runs, keyboard works, `dir` renders a correctly aligned scrolling listing — DirectWrite glyphs, CPU atlas, HLSL `cell_text` instanced draws, ConPTY. DirectWrite face + colour emoji (CBDT via `IDWriteFactory4`, full format set) + HLSL set (`fxc`→DXBC at build time) landed; all `phase1_skeleton` markers gone. Root cause of "nothing renders" was a **missing viewport** — `RSSetViewports` had no callers, and `Clear` ignores the viewport while draws are clipped entirely by it, so it presented as a healthy cleared surface with no D3D error. Criteria unmet: PowerShell/TUI/ligatures/emoji-fallback/kitty-keyboard unverified, vttest not started, forced WARP not retested. Discovery is still the directory scanner (#29). §6 records three instrumentation traps that each produced a retracted conclusion. |

...
| `terminal/` | fork of `microsoft/terminal`, branch `windows` | 42 patches in `terminal-patches/` |
...
scratch clone: 31 and 42 patches, each producing a tree byte-identical to the fork it came
from.
...
.\scripts\build-terminal.ps1 -Project src\host\proxy\Host.Proxy.vcxproj -Configuration Release
.\scripts\build-terminal.ps1 -Project terminal -Configuration Release
.\scripts\build-terminal.ps1 -Project package -Configuration Release -Deploy
```
...
.\scripts\smoke-harness.ps1              # 10 checks
```
...
| 7 | presentation & performance — **in progress**. A ghostty pane drained its pty at 4.2 MB/s against cascadia's 38.1; a forced render per chunk of output was the whole gap, and throttling it reached 28 MB/s median. See `docs/perf/throughput.md`. Presentation itself is untouched. |

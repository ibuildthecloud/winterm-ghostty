...
|---|---|---|---|---|
| 1 | Cursor movements | **pass** | pass | Cursor-control characters inside ESC sequences render the required four identical lines, and the centred E-frame is hollow, correctly positioned and correctly sized. The box is drawn at 80 columns — vttest is a VT100 tester and VT100 is 80 columns — so on a wider terminal it occupies the left 80 columns with space beside it. Confirmed by widening the window: the box stayed at 80. To reproduce wintty's "unbroken border around the full edge" the terminal has to *be* 80 columns. |
...
**None.** Sections 3 and 4 match the baseline exactly, section 1 passes, and section 2 is
assessed here for the first time and passes the part wintty deferred.

Section 1's border initially looked broken; that traced to the PTY size defect above,
which is now fixed. What remains is vttest's own 80-column layout on a wider terminal,
not a drawing fault.
